...
{
    [MenuItem("Battle/Ability/Create New")]
    ...
    private void DrawAttributes(DataDrivenAbility ability)
    {
        ...
        ability.abilityData.ManaCost = EditorGUILayout.FloatField("ManaCost", ability.abilityData.ManaCost);
        ability.abilityData.BehaviorFlags = (BehaviorFlag)EditorGUILayout.EnumFlagsField("Behavior Flags", ability.abilityData.BehaviorFlags);
        ability.abilityData.TargetFlags = (TargetFlag)EditorGUILayout.EnumFlagsField("Can Target", ability.abilityData.TargetFlags);
        ...
        ability.abilityData.AnimationCastPoint = EditorGUILayout.FloatField("AnimCastPoint", ability.abilityData.AnimationCastPoint);

        if ((ability.abilityData.BehaviorFlags & BehaviorFlag.Channelled) != 0)
        {
    ...
    {
        if ((abilityData.BehaviorFlags & BehaviorFlag.AOE) == 0)
        {
            return;
        }
        GUILayout.Label("AOE Attributres", EditorStyles.boldLabel);
        abilityData.AoeData.Center = (CenterType)EditorGUILayout.EnumPopup("Center", abilityData.AoeData.Center);
        abilityData.AoeData.Shape = (ShapeType)EditorGUILayout.EnumPopup("Shape", abilityData.AoeData.Shape);

        if (abilityData.AoeData.Shape == ShapeType.Circle)
        {
            abilityData.AoeData.Radius = EditorGUILayout.IntField("Radius", abilityData.AoeData.Radius);
        }
        else if (abilityData.AoeData.Shape == ShapeType.Rectangle)
        {
    ...
    private void DrawCustomAttributes(AbilityActionData actionData)
    {
        ...
        if (actionData.showCustomAttributes)
        {
            ...
            DrawVec3Field(actionData.Vector3Fields);
            DrawVec2Field(actionData.Vector2Fields);
            DrawStrField(actionData.StringFields);
    ...
    void DrawVec3Field(List<Vector3Field> vec3Fields)
    {
        GUILayout.BeginHorizontal();
        GUILayout.Label("Vector3 Fields", EditorStyles.boldLabel);
        if (GUILayout.Button("+", GUILayout.Width(20)))
        {
            vec3Fields.Add(new Vector3Field());
        }
        GUILayout.Space(280); // hack!
        GUILayout.EndHorizontal();
    ...
    void DrawVec2Field(List<Vector2Field> vec2Fields)
    {
        GUILayout.BeginHorizontal();
        GUILayout.Label("Vector2 Fields", EditorStyles.boldLabel);
        if (GUILayout.Button("+", GUILayout.Width(20)))
        {
            vec2Fields.Add(new Vector2Field());
        }
        GUILayout.Space(280); // hack!
        GUILayout.EndHorizontal();
    ...
    void DrawStrField(List<StringField> strFields)
    {
        GUILayout.BeginHorizontal();
        GUILayout.Label("String Fields", EditorStyles.boldLabel);
        if (GUILayout.Button("+", GUILayout.Width(20)))
        {
            strFields.Add(new StringField());
        }
        GUILayout.Space(280); // hack!
        GUILayout.EndHorizontal();
    ...
    void DrawEvent(AbilityEventData abilityEvent)
    {
        if (abilityEvent.showInEditor)
        {
            ...
            abilityEvent.Type = (EventAbility)EditorGUILayout.EnumPopup(new GUIContent("Type"), abilityEvent.Type);
    ...
    void DrawAction(AbilityActionData eventAction)
    {
        ...
        {
            eventAction.Type = (Action)EditorGUILayout.EnumPopup(new GUIContent("Type", "???"), eventAction.Type);
            eventAction.Target = (Target)EditorGUILayout.EnumPopup("Target", eventAction.Target);
